...
def get_imgc(
    trait: str, resolution: str = "100m", version: str = "v03", year: int = 2020
):
...
def plot_gee_image(
    ...
    predictedImage = get_global_img_mosaic(
        trait, year=year, resolution=resolution, apply_scaling=True
    ).reproject(crs="EPSG:4326", scale=100000)

    # Image mask
    CGIAR_PET = ee.Image("projects/ee-speckerfelix/assets/open-earth/cgiar_pet").select(
        "CGIAR_PET"
    )
...
def get_time_series(trait, geometry, resolution: str = "20m"):
    ...
    collections = {
        "laie": ee.ImageCollection(
            f"projects/ee-speckerfelix/assets/open-earth/laie_predictions-mlp{suffix}"
        ),
...
def plot_single_trait(trait, geom):
    ...
    labels = {"laie": "LAIe", "fapar": "FAPAR", "fcover": "FCOVER"}
...
def get_min_max_scale():
    return_dict = {
        ...
        "laie": {"min": 0, "max": 5.0, "scaling": 1000},
        "fapar": {"min": 0, "max": 1.0, "scaling": 10000},
        "fcover": {"min": 0, "max": 1.0, "scaling": 10000},
        "lai_std": {"min": 0, "max": 2.0},
        "laie_std": {"min": 0, "max": 2.0},
        "fapar_std": {"min": 0, "max": 0.5},
        "fcover_std": {"min": 0, "max": 0.5},
        "count": {"min": 0, "max": 20},
    }
    return return_dict
...
def get_color_palettes():
    ...
    # from black to white for count (white more)
    count_hex = ["#440154", "#3b528b", "#21908d", "#5dc863", "#fde725"]

    count_cmap = mcolors.LinearSegmentedColormap.from_list(
        "custom_cmap", count_hex, N=31
    )
    count_norm = mcolors.Normalize(
        vmin=min_max_scale["count"]["min"], vmax=min_max_scale["count"]["max"]
    )  # Min/max for
    ...
    return {
        ...
        "count_visparams": {
            "min": min_max_scale["count"]["min"],
            "max": min_max_scale["count"]["max"],
            "palette": count_hex,
        },
        "std_hex": std_hex,
        "std_cmap": cmap_std,
        "std_lai_norm": norm_lai_std,
        "std_laie_norm": norm_lai_std,
        ...
        "diff_laie_norm": mcolors.Normalize(vmin=-2, vmax=2),
...
def get_global_img_mosaic(
    ...
    if apply_scaling:
        ...
        band_names_to_scale = [
            f"{trait.lower()}_mean",
            f"{trait.lower()}_stdDev",
            f"{trait.lower()}_stdDev_across",
            f"{trait.lower()}_stdDev_within",
        ]
        return ee.Image(
            [
                img.select(band_names_to_scale)
                .divide(scaling)
                .rename(band_names_to_scale),
...
def get_geometries():
    ...
    point_geoms_fig2 = {
        # "campeche": ee.Geometry.Point([-90.1297730495951, 18.683391639805464]), # figure
        # "congo_basin": ee.Geometry.Point([22.15659692843015, 1.6662957721269671]),
        "destas": destas.centroid(),  # figure 2A
        "congo_basin": ee.Geometry.Point(
            [22.148697900633977, 1.5865866533131594]
        ),  # figure 2C
        "wash": ee.Geometry.Point(
            [-123.93749587447398, 47.33471630778331]
        ),  # figure 2B
    }

    point_geoms_figs3 = {
        "shifting_agriculture": ee.Geometry.Point(
            [20.513602900558794, 1.3027626782150181]
        ),  # figure S3-1
        "olympic_rain_forest": ee.Geometry.Point([-124.289, 47.917]),  # figure S3-2
        "miombo_trop_dry": ee.Geometry.Point(
            [26.61256095983818, -11.55442086704823]
        ),  # figure S3-3
        "temp_deciduous_mosaic_agri": ee.Geometry.Point(
            [10.506193751404401, 49.813215966660124]
        ),  # figure S3-4
    }
    ...
    return {
        ...
        "fire_bbox": fire_bbox,
        "fire_geom": fire_geom,
        "point_geoms_fig2": point_geoms_fig2,
        "point_geoms_figs3": point_geoms_figs3,
        "point_geoms_figs4": point_geoms_figs4,
