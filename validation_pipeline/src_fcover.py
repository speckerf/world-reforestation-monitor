...
def fcover_directories() -> tuple:
    ...
    return (
        ...
        os.path.join(
            ...
            "output",
            "fcover",
            f"merged_fcover_{folder_name}.csv",
        ),
        folder_name,
    )
...
def fcover_points():
    logger.info(
        "Fetching FCOVER validation data from GBOV and adding closest cloudfree S2 image reflectances - export to GCS"
    )

    input, output, foldername = fcover_directories()

    logger.info(
        f"Merging GBOV validation data from folder {input} and saving to {output}"
    )

    merge_fcover_files(input, output)
    ...
    for site in gdf["SITE_ID"].unique():
        ...
        output_filename = (
            f"open-earth/validation/fcover/{foldername}_{site}_reflectances_with_angles"
        )
