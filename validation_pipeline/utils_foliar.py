...
def merge_foliar_files(directories) -> pd.DataFrame:
    ...
    for directory in directories:
        ...
        if len(chl) != 1 or len(sample) != 1 or len(lma) != 1:
            logger.warning(f"Expected 1 file for each pattern, found {len(chl)} files for  and {len(sample)} files for sample in folder {directory}")
            continue
        
        cfc_chl_fields = ['sampleID', 'extractChlAConc', 'freshMass',
                          'extractChlBConc', 'extractCarotConc', 
                          'solventVolume', 
                          'sampleCondition', 'dataQF',
                          'handlingQF', 'measurementQF']
        cfc_fieldData_fields = ['plotID', 'sampleID', 'nlcdClass', 
                                'decimalLatitude', 'decimalLongitude', 'sampleType', 
                                'scientificName', 'crownPolygonID', 'collectDate']
        cfc_lma_fields = ['sampleID', 'lmaSampleCondition', 'freshMass',
                          'dryMass', 'leafArea', 'percentGreen', 
        ...
        joined_df = pd.merge(df_chl, df_sample, on='sampleID', how='inner').merge(df_lma, on='sampleID', how='inner', suffixes=('_chl', '_lma'))
        
        # remove all columns with dataQF not NAN, and sampleCondition not 'OK'
        filtered_df = joined_df[(joined_df['dataQF'].isna()) & 
        ...
        # filterings
        group_filtered_df = (grouped_df
                                .filter(lambda x: len(x['decimalLatitude'].unique()) == 1 and
                                        ((x['collectDate'].max() - x['collectDate'].min()).days <= 10))
                            )
                            
        # calculate group_mean values of traits:
        mean_df = group_filtered_df.groupby('plotID').agg({'extractChlAConc': 'mean', 'extractChlBConc': 'mean', 'extractCarotConc': 'mean',
                                                           'decimalLatitude': 'first', 'decimalLongitude': 'first', 'collectDate': 'median',
                                                           'freshMass_chl': 'mean', 'freshMass_lma': 'mean', 'leafMassPerArea': 'mean', 'dryMass': 'mean', 'leafArea': 'mean',
                                                           'solventVolume': 'mean', 'dryMassFraction': 'mean', 'percentGreen': 'mean'})
