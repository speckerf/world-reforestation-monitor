...
def merge_fapar_files(directory, output_filename):
    ...
    combined_data = pd.concat(
        [
            process_fapar_csv_file(f)
            for f in csv_files
            if process_fapar_csv_file(f) is not None
        ],
        ignore_index=True,
    )
...
def process_fapar_csv_file(file_path):
    try:
        ...
        if not (
            all(col in df.columns for col in ["FIPAR_up", "FIPAR_up_err"])
            or all(col in df.columns for col in ["FIPAR_down", "FIPAR_down_err"])
            or all(
                col in df.columns
                for col in ["FIPAR_up", "FIPAR_up_err", "FIPAR_down", "FIPAR_down_err"]
            )
        ):
            logger.error(f"Missing FIPAR columns in file: {file_path}")
            return None  # Return None for problematic files
        ...
        if all(
            ...
            # Calculate total FIPAR: FIPAR = FIPAR_{up} + (1 - FIPAR_{up}) FIPAR_{down}
            df["FIPAR_total"] = df["FIPAR_up"] + (1 - df["FIPAR_up"]) * df["FIPAR_down"]

            # Calculate total FIPAR error: FIPAR_err = /sqrt{(1 - FIPAR_{down})^2 FIPAR_{up_err}^2 + (1 - FIPAR_{up})^2 FIPAR_{down_err}^2}
            df["FIPAR_total_err"] = (
                ((1 - df["FIPAR_down"]) ** 2 * df["FIPAR_up_err"] ** 2)
                + ((1 - df["FIPAR_up"]) ** 2 * df["FIPAR_down_err"] ** 2)
            ) ** 0.5

        elif all(col in df.columns for col in ["FIPAR_up", "FIPAR_up_err"]):
            df["FIPAR_total"] = df["FIPAR_up"]
            df["FIPAR_total_err"] = df["FIPAR_up_err"]
        elif all(col in df.columns for col in ["FIPAR_down", "FIPAR_down_err"]):
            df["FIPAR_total"] = df["FIPAR_down"]
            df["FIPAR_total_err"] = df["FIPAR_down_err"]
        else:
            logger.error(f"Missing FIPAR columns in file: {file_path}")
            return None
